...
def update_plot(pararameters):
    ...
    # Update title with lightcurve data
    ax1_title.set_text(f'Min:${min(intensity):.1f}$   Max:${max(intensity):.1f}$   \
    Avg:${np.mean(intensity):.1f}\\pm{np.std(intensity):.1f}$')
...
def clear_axes(event):
    ...
    sld_tbin.val = nbins//2
    sld_ecut.val = nbins//2
    parameters[0] = timebins[sld_tbin.val]
    parameters[1] = energies[sld_ecut.val]
...
if __name__ == "__main__":
    ...
    gti0 = create_gti_from_condition(lc_raw.time, lc_raw.countrate > 1)
    events.gti = gti0
    ...
    instrument = events.instr
    tstart = events.mjdref + min(lc_raw.time)/86400


    if mission.lower()=='nustar':
        emin = 3
        emax = 79
        bint = 50   ## binning for static lightcurve
        nbins = 15
        energies = np.geomspace(4, 30, nbins, endpoint=True)
        timebins = np.geomspace(1,200, nbins, endpoint=True)
        idx = [100//bint, -100//bint]
        ...
        bint = 15
        nbins = 15
        energies = np.geomspace(0.8,8, nbins, endpoint=True)
        timebins = np.geomspace(1, 30, nbins, endpoint=True)
        idx = [50//bint, -50//bint]
        ...
        bint = 50
        nbins = 15
        energies = np.geomspace(1,8, nbins, endpoint=True)
        timebins = np.geomspace(1, 200, nbins, endpoint=True)
        idx = [100//bint, -100//bint]
    ...
    fig = plt.figure(figsize=plt.figaspect(0.6), dpi=160, constrained_layout=True)
    gs = gridspec.GridSpec(2,4, wspace=0.075, hspace=0.075, 
            height_ratios=[0.6,1], width_ratios=[0.04,1,1,0.04])

    ax0 = fig.add_subplot(gs[0, :])
    ax1 = fig.add_subplot(gs[1, 1])
    ax2 = fig.add_subplot(gs[1, 2], sharey=ax1)

    ax0.tick_params(size=0, top=False, labelbottom=False, labeltop=False, 
    labelsize='small')
    ax1.tick_params(size=0, which='both', labelleft=False, labelright=False,
     labelsize='small')
    ax2.tick_params(size=0, which='both', labelleft=False, labelright=False,
     labelsize='small')

    ax0_top = ax0.secondary_xaxis('top')
    ax0_top.tick_params(pad=1, direction='in', size=2, which='both', 
    labelsize='small')
    ax0_top.set_xlabel(f'Time (ks since ${tstart:.1f}$ MJD; ${bint:g}$s bin)',
     fontsize='small')
    ...
    ax2.set_xscale('log')
    ax2.minorticks_off()
    ax2.axvline(1, lw=1.5, c='k', zorder=1)

    # Title container for updating parameters
    ax1_title = ax1.set_title('', fontsize='small', loc='center', pad=1)

    ## Create static lightcurve
    ## -------------------------

    total_events = events.filter_energy_range([emin,emax])
    total_lc = total_events.to_lc(bint)
    gti = create_gti_from_condition(total_lc.time, total_lc.counts>1)
    total_lc.gti = gti
    total_lc.apply_gtis()
    ...
    split_lc = total_lc.split(min_gap=bint, min_points=3)
    tmin = min(split_lc[0].time)/1000

    times = []
    for k in range(len(split_lc)):
        if len(split_lc[k].time[idx[0]:idx[1]])<3 : continue
        ax0.step(split_lc[k].time[idx[0]:idx[1]]/1000-tmin,
        split_lc[k].countrate[idx[0]:idx[1]], 
        where='mid', lw=1, c='k', zorder=2)
        times.append([split_lc[k].time[idx[0]], split_lc[k].time[idx[1]]])


    # Snap intervals for time selection
    ...
    scat1 = ax1.scatter([],[], c=[], cmap='viridis', marker='s', s=13, zorder=3)
    scat2 = ax2.scatter([],[], c=[], cmap='viridis', marker='s', s=13, zorder=3)
    ...
    timespan = SpanSelector(ax0, read_span_selector, direction="horizontal",
            ...
            handle_props=dict(lw=1.25, ls='--'), interactive=True, 
            drag_from_anywhere=True, ignore_event_outside=True,
	        grab_range=10, snap_values=snap_times )


    ax_ecut = fig.add_subplot(gs[1, 3])
    ax_tbin = fig.add_subplot(gs[1, 0])
    ...
    sld_ecut.valtext.set_visible(False)
    sld_tbin.valtext.set_visible(False)
    
    ## Reset button
    ## ------------- 
    ax_reset = fig.add_axes([0, 0, 0.06, 0.06])
